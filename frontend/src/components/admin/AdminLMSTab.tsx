  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-[var(--color-gold)]/30 transition-all">
      <div className="relative h-40 bg-zinc-800">
        ...
        <div className={`absolute top-3 right-3 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${course.isPublished ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-zinc-700 text-zinc-400 border border-zinc-600'
          }`}>
      ...
      <div className="p-5">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => onTogglePublish(course)} className={`px-3 py-2 rounded-xl text-xs font-black border ${course.isPublished ? 'border-red-800 text-red-400' : 'border-green-800 text-green-400'
            }`}>
            ...
            <div key={mod.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  {mod.videoUrl ? (
                    <span className={`inline-flex items-center gap-1 text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${mod.videoType === 'YOUTUBE' ? 'bg-red-900/30 text-red-400' : 'bg-blue-900/30 text-blue-400'
                      }`}>
              ...
              <div className="flex rounded-xl overflow-hidden border border-zinc-700">
                {["YOUTUBE", "CLOUDINARY"].map(type => (
                  <button key={type} onClick={() => setForm(f => ({ ...f, videoType: type, videoUrl: "" }))}
                    className={`flex-1 py-2.5 text-xs font-black flex items-center justify-center gap-2 transition-all ${form.videoType === type ? 'bg-[var(--color-gold)] text-zinc-900' : 'bg-zinc-800 text-zinc-400 hover:text-white'
                      }`}>
                ...
                {form.options.map((opt, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    <button onClick={() => setForm(f => ({ ...f, correctOption: idx }))}
                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${form.correctOption === idx ? 'border-green-500 bg-green-500' : 'border-zinc-600'
                        }`}>
                    ...
                    <input value={opt} onChange={e => { const arr = [...form.options]; arr[idx] = e.target.value; setForm(f => ({ ...f, options: arr })); }}
                      placeholder={`Option ${idx + 1} *`} className={`flex-1 bg-zinc-800 border rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none transition-all ${form.correctOption === idx ? 'border-green-700/50 focus:border-green-500/60' : 'border-zinc-700 focus:border-[var(--color-gold)]/50'
                        }`} />
                  </div>
                ))}
              </div>
        ...
        <div className="flex gap-3 bg-zinc-900 rounded-2xl p-1 border border-zinc-800 w-fit">
          {(["modules", "quiz"] as const).map(v => (
            <button key={v} onClick={() => setManageView(v)}
              className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 ${manageView === v ? 'bg-[var(--color-gold)] text-zinc-900' : 'text-zinc-400 hover:text-white'
                }`}>
